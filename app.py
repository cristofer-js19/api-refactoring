...
def get_detalhes_do_leilao(id_leilao):
  with db.conexao_gerenciada().cursor() as cur:
    ...
    ultimo_lance = None if len(lances) == 0 else lances[-1]
    return jsonify({
      'id': leilao[0],
      'descricao': leilao[1],
      'criador': leilao[2],
...
def registrar_lance(id_leilao):
  ...
  id_usuario = request.headers['X-Id-Usuario'] # simulação de autenticação ultra simplificada
  
  with db.conexao_gerenciada().cursor() as cur:
    try:
      modelo.leilao.registrar_lance(cur, id_leilao, id_usuario, dados['valor'])
    except modelo.leilao.LanceMenorQueUltimo:
      return 'Lance deve ser maior que o último.', HTTPStatus.BAD_REQUEST
    except modelo.leilao.LanceMenorQueDiferencaMinima:
      return 'Lance deve ser maior que o atual mais a diferença mínima.', HTTPStatus.BAD_REQUEST
    except modelo.leilao.CriadorNaoPodeDarLance:
...
def registrar_lance_minimo(id_leilao):
  ...
  with db.conexao_gerenciada().cursor() as cur:
    ...
    ultimo_lance = repositorio.leilao.buscar_valor_ultimo_lance(cur, id_leilao)
    diferenca_minima = repositorio.leilao.buscar_diferenca_minima(cur, id_leilao)
    valor = 1 if ultimo_lance is None else ultimo_lance + diferenca_minima
    try:
      modelo.leilao.registrar_lance(cur, id_leilao, id_usuario, valor)
    except modelo.leilao.CriadorNaoPodeDarLance:
      return 'Criador não pode dar lance.', HTTPStatus.BAD_REQUEST      
  return '', HTTPStatus.NO_CONTENT
